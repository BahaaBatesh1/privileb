//
//  AboutPagerViewController.swift
//  Privileb
//
...
import UIKit
protocol AboutPagerProtocol{
    func didChangeTab(position: Int)
}

class AboutPagerViewController: PagerController, PagerDataSource, PagerDelegate {
    var tabChangedDelegate : AboutPagerProtocol?
    ...
    func controllerForTabAtIndex(index: Int, pager: PagerController) -> UIViewController {
        if(index == 0){
            let controller = self.storyboard?.instantiateViewController(withIdentifier: "englishAbout") as! EnglishAboutViewController
            return controller
            
        } else {
            //Walks
            let controller = self.storyboard?.instantiateViewController(withIdentifier: "arabicAbout") as! ArabicAboutViewController
            return controller
